...
for every in (50, 40):
    R.install(0.0, every, True, True, "off", False, 1.0, 1, "thread", "latest")
    f = Fake(); rows = []; blocks = []
    ...
    print(f"every={every}: " + " | ".join(f"청크{c}: 인덱스 {a}~{b} ({n}스텝)" for c, a, b, n in runs))
    print(f"          제어 루프가 막힌 스텝(첫 스텝 제외): {blocks if blocks else '없음'}")
